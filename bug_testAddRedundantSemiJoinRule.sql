...
SELECT 1 FROM (values (0, '', '', 0, 0, 0, 0, 0, 0)) AS EMP INNER JOIN (values (0, ''),(0, '')) AS DEPT ON EMP.DEPTNO = DEPT.DEPTNO;
SELECT 1 FROM (values (0, '', '', 0, 0, 0, 0, 0, 0)) AS EMP0 INNER JOIN (values (0, ''),(0, '')) AS DEPT0 ON EMP0.DEPTNO = DEPT0.DEPTNO INNER JOIN (values (0, ''),(0, '')) AS DEPT1 ON EMP0.DEPTNO = DEPT1.DEPTNO;
